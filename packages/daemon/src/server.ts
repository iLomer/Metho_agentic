...
import type { DashboardMessage, DaemonMessage } from "./types.js";
...
/**
 * Handles a single message from the dashboard.
 */
...
export function startServer(port: number = DEFAULT_PORT): void {
  ...
  process.stdout.write(`  waiting for dashboard connection...\n\n`);

  wss.on("connection", (ws) => {
    process.stdout.write(`  dashboard connected\n`);
    ...
    ws.on("close", () => {
      process.stdout.write(`  dashboard disconnected\n`);
    });
